...
<template>
  ...
  >
    ...
    >
      ...
      <div class="flex-c gap-2.5">
        ...
        <ArtIconButton
          ...
          @click="visibleNotice"
        >
          <div v-if="noticeUnread" class="absolute top-2 right-2 size-1.5 !bg-danger rounded-full" />
        </ArtIconButton>
    ...
    <!-- 通知 -->
    <ArtNotification v-model:value="showNotice" @read-all="noticeUnread = false" ref="notice" />
  </div>
</template>
...
<script setup lang="ts">
  ...
  // 顶部栏功能配置（Dojo 只保留：菜单、刷新、面包屑、通知、日夜切换、用户菜单）
  const {
