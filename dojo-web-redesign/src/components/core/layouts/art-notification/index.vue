...
<script setup lang="ts">
  ...
  import { dojoNotices, dojoTodos } from '@/mock/dojo/notifications'
  ...
  type NoticeType = 'confirm' | 'overdue' | 'account' | 'publish' | 'agent'
  ...
  const useNotificationData = () => {
    ...
    const noticeList = ref<NoticeItem[]>(
      dojoNotices.map((n) => ({ title: n.title, time: n.time, type: n.type as NoticeType }))
    )

    const msgList = ref<MessageItem[]>([])

    const pendingList = ref<PendingItem[]>(
      dojoTodos.map((t) => ({ title: t.title, time: t.time }))
    )
...
<style scoped>
  ...
  .art-notification-panel {
    ...
    box-shadow:
      ...
      0 1px 3px rgb(0 0 0 / 6%) !important;
